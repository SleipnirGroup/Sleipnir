...
TEST_CASE("Expression - is_constant", "[Expression]") {
  // Constant zero
  auto zero = constant_ptr(0.0);
  // NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
  CHECK(zero->is_constant(0.0));
  // NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
  CHECK_FALSE(zero->is_constant(1.0));

  // Constant one
  auto one = constant_ptr(1.0);
  CHECK_FALSE(one->is_constant(0.0));
  CHECK(one->is_constant(1.0));

  // Linear variable
  auto x = make_expression_ptr<DecisionVariableExpression>(1.0);
  CHECK_FALSE(x->is_constant(0.0));
  CHECK_FALSE(x->is_constant(1.0));
}

TEST_CASE("Expression - constant_ptr", "[Expression]") {
  CHECK(constant_ptr(0.0)->is_constant(0.0));
}

TEST_CASE("Expression - Prune multiply", "[Expression]") {
  auto zero = constant_ptr(0.0);
  auto one = constant_ptr(1.0);
  auto two = constant_ptr(2.0);
...
TEST_CASE("Expression - Prune cbrt()", "[Expression]") {
  auto negative_one = constant_ptr(-1.0);
  auto zero = constant_ptr(0.0);
  auto one = constant_ptr(1.0);
  auto two = constant_ptr(2.0);
...
TEST_CASE("Expression - Prune sign()", "[Expression]") {
  ...
  CHECK(slp::detail::sign(constant_ptr(-2.0))->is_constant(-1.0));
