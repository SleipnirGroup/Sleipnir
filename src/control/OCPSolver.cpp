...
FixedStepOCPSolver::FixedStepOCPSolver(int numStates, int numInputs, double dt,
                                       int numSteps,
...
void FixedStepOCPSolver::ConstrainDirectCollocation() {
  ...
  for (int i = 0; i < m_numSteps; ++i) {
    ...
    auto xprime_c =
        (-3.0 / (2.0 * m_dt)) * (x_begin - x_end) - (f_begin + f_end) / 4.0;
...
void FixedStepOCPSolver::ConstrainDirectTranscription() {
  for (int i = 0; i < m_numSteps; ++i) {
    ...
    if (m_dynamicsType == kExplicitODE) {
      SubjectTo(
          x_end ==
          RK4<const DynamicsFunction&, VariableMatrix, VariableMatrix, double>(
