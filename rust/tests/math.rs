...
fn unary_math() {
    ...
    assert!(close(math::sqrt(Variable::constant_in(&arena, 9.0)).value(), 3.0));
    assert!(close(math::abs(Variable::constant_in(&arena, -2.5)).value(), 2.5));
    assert!(close(math::cbrt(Variable::constant_in(&arena, 27.0)).value(), 3.0));
...
fn binary_math() {
    let arena = VariableArena::new();
    assert!(close(math::pow(Variable::constant_in(&arena, 2.0), 10.0).value(), 1024.0));
    // hypot with f64 `x` isn't allowed (no arena); pass x as a Variable.
    assert!(close(
        math::hypot(Variable::constant_in(&arena, 3.0), 4.0).value(),
        5.0
    ));
    assert!(close(math::max(Variable::constant_in(&arena, 1.0), 3.0).value(), 3.0));
    assert!(close(math::min(Variable::constant_in(&arena, 1.0), 3.0).value(), 1.0));
    assert!(close(math::atan2(Variable::constant_in(&arena, 0.0), 1.0).value(), 0.0));
}
